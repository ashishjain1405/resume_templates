...
function ATSCheckInner() {
  ...
  async function handleAnalyse() {
    ...
    try {
      ...
      if (currentTab === 'upload' && currentFile) {
        // Read bytes first — on Android Chrome the File handle can become unreadable
        // after extractPdfText consumes it via arrayBuffer().
        fileBuffer = await currentFile.arrayBuffer()
        try {
          const text = await extractPdfText(new Blob([fileBuffer], { type: 'application/pdf' }))
      ...
      if (currentTab === 'upload' && fileBuffer && !selectedResumeIdRef.current) {
        const uploadFile = new File([fileBuffer], currentFile?.name ?? 'resume.pdf', { type: 'application/pdf' })
        const uploadForm = new FormData()
  ...
  async function handleAIRewrite(templateIdOverride?: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(payload.error === 'rate_limited' ? 'You\'ve reached the limit of 5 rewrites per hour. Please try again later.' : (payload.error ?? 'Rewrite failed. Please try again.'))
        return
      }
      sessionStorage.setItem('rewrite_result', JSON.stringify({
        originalData: payload.originalData,
        originalScore: { overall_score: result!.overall_score, ats_score: result!.ats_score, recruiter_score: result!.recruiter_score },
        rewrittenData: payload.rewrittenData,
        rewrittenScore: payload.rewrittenScore,
            ...
            <div className="border border-gray-100 rounded-2xl p-6 shadow-sm space-y-6">
              <div className="flex gap-2 flex-wrap">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-semibold text-sm transition-colors disabled:opacity-60 ${!isPro ? 'bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                >
                  {rewriteLoading ? (
                  ...
                  {!isPro && !rewriteLoading && <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 1a4.5 4.5 0 00-4.5 4.5V9H5a2 2 0 00-2 2v6a2 2 0 002 2h10a2 2 0 002-2v-6a2 2 0 00-2-2h-.5V5.5A4.5 4.5 0 0010 1zm3 8V5.5a3 3 0 10-6 0V9h6z" clipRule="evenodd" /></svg>}
                </button>
                {builderTemplateId ? (
                  <Link
                    href={`/builder/${builderTemplateId}`}
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-semibold text-sm transition-colors border border-gray-200 text-gray-700 bg-white hover:bg-gray-50"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl font-semibold text-sm transition-colors disabled:opacity-50 ${isPro ? 'border border-gray-200 text-gray-700 bg-white hover:bg-gray-50' : 'bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100'}`}
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
